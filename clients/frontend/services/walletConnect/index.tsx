...
import { AccountBalances, ChainNamespaces, getAllChainNamespaces } from './helpers/index';
/**
 * Types
 */
interface IContext {
 ...
 * Provider
 */
export function WCClientContextProvider({ children }: { children: ReactNode | ReactNode[] }) {
  ...
  console.log('wc isConnectReady', isConnectReady);
  const resetApp = () => {
    setPairings([]);
  ...
  const _subscribeToProviderEvents = useCallback(
    async (_client: UniversalProvider) => {
      ...
      _client.on('session_update', ({ topic, session }: { topic: string; session: SessionTypes.Struct }) => {
  ...
  const connect = useCallback(
    async (caipChainId: string, pairing?: { topic: string }) => {
      try {
        ...
        console.log('wc  connect catch', err);
        throw err;
      }
    },
