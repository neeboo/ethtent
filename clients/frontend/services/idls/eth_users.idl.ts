export const idlFactory = ({ IDL }) => {
  ...
  const ChainType = IDL.Variant({ BTC: IDL.Null, EVM: IDL.Null });
  ...
  return IDL.Service({
    ...
    ensureSaltSet: IDL.Func([], [IDL.Vec(IDL.Nat8)], []),
    getDelegation: IDL.Func([GetDelegationRequest], [GetDelegationResponse], ['query']),
    secureMessage: IDL.Func([ChainType, IDL.Bool], [Result_6], []),
    setLoginPrefix: IDL.Func([ChainType, IDL.Text], [], []),
    verifyMessage: IDL.Func([VerifyMessagePayload], [Result_7], []),
