...
export type OrderType = { BuyLimit: null } | { Platform: null } | { BuyStop: null } | { SellStop: null } | { SellLimit: null } | { Market: null };
export interface PlatformDetail {
  platform_uuid: string;
  key_name: string;
  chain_type: ChainType;
}
export type Result = { Ok: UserIntents } | { Err: string };
export type Result_1 = { Ok: AppInfo } | { Err: string };
export type Result_2 = { Ok: null } | { Err: string };
export type Result_3 = { Ok: Array<[string, Array<Principal>]> } | { Err: string };
export type Result_4 = { Ok: boolean } | { Err: string };
export type Result_5 = { Ok: Array<LogEntry> } | { Err: string };
export type Result_6 = { Ok: [] | [Array<[Principal, string]>] } | { Err: string };
export type Result_7 = { Ok: [Principal, string] } | { Err: string };
export type Result_8 = { Ok: string } | { Err: WalletError };
export type Result_9 = { Ok: AddressInfo } | { Err: WalletError };
export type RoleFilter = { All: null } | { RoleAnd: [OrderType, OrderType] } | { RoleNot: OrderType } | { Role: OrderType };
