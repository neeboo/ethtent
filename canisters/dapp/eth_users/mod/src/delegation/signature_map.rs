...
impl SignatureMap {
    pub fn put(&mut self, seed: Hash, message: Hash, signature_expires_at: u64) {
        if self.certified_map.get(&seed[..]).is_none() {
            let mut sub_map = RbTree::new();
            sub_map.insert(message, Unit);
            self.certified_map.insert(seed, sub_map);
        } else {
            self.certified_map.modify(&seed[..], |sub_map| {
                sub_map.insert(message, Unit);
            });
        }
